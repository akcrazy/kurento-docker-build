...
if [[ -n "${KMS_UID:-}" && "$KMS_UID" != "$RUN_UID" ]]; then
    ...
    echo "[Docker entrypoint]-2 Start Kurento Media Server, UID: $RUN_UID"

    # Run KMS as PID 1 (replace the current process)
    exec /kms-omni-build/build-Debug/kurento-media-server/server/kurento-media-server --modules-path='/kms-omni-build/build-Debug:/usr/lib/x86_64-linux-gnu/kurento/modules' --gst-plugin-path='/kms-omni-build/build-Debug:/usr/lib/x86_64-linux-gnu/gstreamer-1.5' "$*"
fi
